...
export default function JackpotGame() {
  ...
  useEffect(() => {
    ...
    const total = samplePlayers.reduce((sum, player) => {
      return (
        sum + player.items.reduce((itemSum, item) => itemSum + item.value, 0)
      );
    }, 0);
  ...
  const totalItems = players.reduce(
    (sum, player) => sum + player.items.length,
    0
  );
  ...
  const generateWheelSegments = () => {
    ...
    return players.map((player) => {
      ...
      const innerStartX =
        50 + 20 * Math.cos((startAngle - 90) * (Math.PI / 180));
      const innerStartY =
        50 + 20 * Math.sin((startAngle - 90) * (Math.PI / 180));
                ...
                <div className="flex-1 flex flex-col items-center justify-center bg-[#2D0845]/80 rounded-xl p-6 border-2 border-[#4D1A74] shadow-[0_0_20px_rgba(156,44,243,0.3)] relative overflow-hidden">
                        ...
                        <div className="mt-2 flex items-center justify-center space-x-4">
                          ...
                          {gameState === "waiting" && (
                            <div className="flex items-center bg-[#4D1A74]/50 px-3 py-1 rounded-full">
                              ...
                              <span className="text-sm text-white countdown">
                                {timeLeft}s
                              </span>
                            </div>
                          )}
                  ...
                  <div className="relative mb-6 w-full max-w-md aspect-square">
                        ...
                        <defs>
                          ...
                          {players.map((player) => (
                            ...
                            >
                              ...
                              <stop
                                offset="100%"
                                stopColor={adjustColor(player.color, -30)}
                              />
                            </linearGradient>
                          ))}
                          ...
                          <g key={`segment-${segment.player.id}`}>
                            <path
                              ...
                              className={`wheel-segment ${
                                highlightedSegment === segment.player.id
                                  ? "segment-highlight"
                                  : ""
                              } ${
                                winner &&
                    ...
                    <div className="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-[60%] z-50 landing-indicator">
                      <div
                        className={`prominent-arrow ${
                          gameState === "spinning" ? "arrow-spinning" : ""
                        }`}
                      >
                        ...
                        {gameState === "complete" && (
                          <div className="arrow-highlight-pulse"></div>
                        )}
                      </div>
                    </div>
